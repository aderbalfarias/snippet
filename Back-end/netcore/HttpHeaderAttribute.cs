...
public class HttpHeaderAttribute : ActionFilterAttribute
{
    ...
    public override void OnActionExecuting(ActionExecutingContext context)
    {
        ...
        if (StringValues.IsNullOrEmpty(headerValue))
        {
            if (this.Required)
            {
                var message = $"{this.HttpHeaderName} HTTP header is required, but was not specified.";
                LogInformation(context, message);
                context.Result = new BadRequestObjectResult(new MissingHeaderError(message));
            }
        }
        else
        {
            ...
            if (this.Required && !this.IsValid(headerValue))
            {
                ...
                LogInformation(context, message);
                context.Result = new BadRequestObjectResult(new InvalidHeaderValueError(message));
            }
        }
    }
    ...
    private static void LogInformation(ActionExecutingContext context, string message)
    {
        ...
        var logger = factory.CreateLogger<HttpHeadAttribute>();
